...
@Component({
  selector: 'app-quick-view-scale',
  templateUrl: './quick-view-scale.component.html',
  styleUrls: ['./quick-view-scale.component.scss']
})
export class QuickViewScaleComponent implements OnInit {
  d;
  eventMap = {
    "desktop": {
      click: "click",
  ...
  restartFlashView() {
    ...
    let _is_zooming_transform,
        _is_zooming_transform_no_scale;
      ...
      timer_detect_gesture = setTimeout(() => {
        ...
        if ( container_target !== null ) {
          ...
          let removeEffect = () => {
            ...
            // view_modal_el.style.cssText = _is_zooming_transform_no_scale;
          ...
          if ( container_target.getAttribute(opts.data_container) !== 'on' && distanceY_Doc === 0 ) {
            ...
            let minusByRatio = (widthRatio - 1) * window.innerWidth;
            let minusHeightByScale = window.innerHeight - minusByRatio;
            console.log('minusByRatio: ', minusByRatio);
            

            view_modal_el.classList.add(opts.is_pressed_cls);
            _is_zooming_transform = `transform: translate(${translateLeft}px, ${translateTop}px) scaleX(${widthRatio});width: ${window.innerWidth}px;height: ${window.innerHeight - minusByRatio}px;`;

            _is_zooming_transform_no_scale = `transform: translate(${translateLeft}px, ${translateTop}px);height: ${window.innerHeight}px;`;
